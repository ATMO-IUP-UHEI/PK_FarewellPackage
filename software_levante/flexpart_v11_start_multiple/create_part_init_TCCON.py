...
def create_part_init(outpath,num_part,num_layers,num_releases, lat,lon,release_time,pmin,pmax,nspecies,species_id,species_mass,kindz,averaging_kernel,pressure_levels):
    ...
    for i in range(0,num_releases):
        ...
        fraction=np.zeros(n_layers)
        for k in range(n_layers):
            if k==n_layers-1:
                fraction[k]=averaging_kernel[i][k]/sum(averaging_kernel[i])*((pressure_levels[i][k])/pressure_levels[i][0])
            else:
                fraction[k]=averaging_kernel[i][k]/sum(averaging_kernel[i])*((pressure_levels[i][k]-pressure_levels[i][k+1])/pressure_levels[i][0])
            
        fraction=fraction/sum(fraction)
        particles=np.array([])
        for k in reversed(range(0,n_layers)):
            temp=np.array([pressure_levels[i][k]]*round(fraction[k]*40000))
            if k==0:
                temp=np.array([pressure_levels[i][k]]*(40000-len(particles)))
            if len(temp)!=0:
                particles=np.concatenate([particles,temp])

        if len(particles)!=40000:
            particles=particles[(len(particles)-40000):]
