...
def main(config_path):
    ...
    if flex_extract_download:
        ...
        if idnum is None:
            idnum = '000'
        if 'ANOG__ML' in fe_files:
            # create restart_dir
            os.makedirs(f'{outpath}restartFailedANOG__ML')
        for fe_file in fe_files:
            ...
            if 'ANOG__ML' in fe_file:
                Wait_for_Jobs(processID, 'ANOG__ML')
                restart_dir=f'{outpath}restartFailedANOG__ML'
                restart_files=[f for f in listdir(restart_dir) if isfile(join(restart_dir, f)) if f.startswith('restart_')]
                while len(restart_files)>0:
            ...
            if 'FCOG' in fe_file:
                ...
                for day in timerange:
                    ...
                    if ymdn.day==1:
                        print(f'Adjusting forecast fiels for {ymdn.strftime("%Y-%m-%d")}')
                        for par in dfFiles[(dfFiles.fefile == fe_file)].paramlist.values[0]:
                            filename=f"E5{file_name}_{ymdn}"
                            outfile=outpath+fe_file+filename+'_'+str(par)+'.grb'
                            # adjust for first of month
...
def convertData(ymdi, ymdf, outpath, res, borders, fe_file,idnum, processID,dfFiles):
    ...
    if 'FCOG' in fe_file: # FCOG files start one day before and extend to two days after the time period
    ...
    for day in timerange:
        ...
        for par in dfFiles[(dfFiles.fefile == fe_file)].paramlist.values[0]:
            ...
            if 'ANOG__ML' in fe_file:#start sbatch jobs for each day and paramter to run parallel in background, writes config file that is deleted after succesfull sbatch job
                restart_outfile=f'{outpath}restartFailedANOG__ML/restart_{filepath[-28:-4]}.yaml'
                print('converting paramter '+str(par)+' for the day '+ ymdn_str +' in background')
                write_ANOG__ML_config(restart_outfile, filepath,res,inter_res,outfile,par,borders,processID,ymdn_str)
                subprocess.run(["sbatch",f"--export=filepath={filepath},resol={res},inter_res={inter_res},outfile={outfile},par={par},borders0={borders[0]},borders1={borders[1]},borders2={borders[2]},borders3={borders[3]},restartfile={restart_outfile}",f"--job-name={processID}","ConvertANOG__ML.sh"])
            else:
...
def Wait_for_Jobs(processID, WaitForFiles):
    '''
    # wait until all subprocess jobs files are finished
    # arguments:
    #            processID: processID of overall Start_ConvertEra5.sh skript, used as job name for subprocess that should be waited for
    #            WaitForFiles (str): files being waited for
    '''
    running = True    
    while running == True:
        time.sleep(30)
        ...
        if 'Finished' in result.stdout:
            if 'running' in result.stdout: # just a test
                ...
                running = False
                print(f'all {WaitForFiles} files are created')
        else:
            print(f'waiting for {WaitForFiles} files to be created, status of jobs will be checked again in 30 sec.')

def write_ANOG__ML_config(restart_outfile, filepath: str,res: float,inter_res: int,outfile: str,par: int,borders:list[float],processID:int,ymdn_str:int):
    """Writes parameters needed for convert_ANOG_ML.py into file, so that conversion can be restarted for failed processes""" 
    content = ""
    content += f"restart_outfile: {restart_outfile}\n"
    content += f"filepath: {filepath}\n"
    content += f"res: {res}\n"
    content += f"inter_res: {inter_res}\n"
    content += f"outfile: {outfile}\n"
    content += f"par: {par}\n"
    content += f"borders: {borders}\n"
    content += f"processID: {processID}\n"
    content += f"ymdn_str: {ymdn_str}\n"
    with open(restart_outfile, "x") as f:
        f.write(content)
        
def readRestartANOG__ML_yaml(config_path):
    with open(config_path, 'r') as f:
        config = yaml.load(f, Loader=yaml.SafeLoader)
        return config['restart_outfile'], config['filepath'], config['res'], config['inter_res'], config['outfile'], config['par'], config['borders'], config['processID'], config['ymdn_str']

def rerunFailedANOG__ML(restart_dir,restart_files):
    for file in restart_files:
        # read ANOG__ML_config
        restart_outfile, filepath,res,inter_res,outfile,par,borders,processID,ymdn_str=readRestartANOG__ML_yaml(f'{restart_dir}/{file}')
        print('restarted converting paramter '+str(par)+' for the day '+ str(ymdn_str) +' in background')
        # start subprocess
        subprocess.run(["sbatch",f"--export=filepath={filepath},resol={res},inter_res={inter_res},outfile={outfile},par={par},borders0={borders[0]},borders1={borders[1]},borders2={borders[2]},borders3={borders[3]},restartfile={restart_outfile}",f"--job-name={processID}","ConvertANOG__ML.sh"])
