...
import yaml
import create_part_init_TCCON as create_part_init
import click
import pandas as pd
...
# python prepare_TCCONruns.py --config_path configs/options_config_TCCON.yaml
###################################################
...
def main(config_path):
    ...
    sounding_dir= config['TCCON_measurements_path']
    ...
    for date in pd.date_range(start_date, end_date):
        ...
        soundings_path=f'{sounding_dir}/{date.strftime("%Y_%m")}/TCCON_{date.strftime("%Y%m%d")}.nc'
        # check that file exists
        if os.path.isfile(soundings_path):
            # with satelight_sounding path
            print(f'reading measurements from {soundings_path}')
            ds=xr.open_dataset(soundings_path)
            sounding_datetime,latitude_temp,longitude_temp, averaging_kernel_temp,pressure_levels_temp=ds.time.values.flatten(),ds.latitude.values,ds.longitude.values,ds.ak_xco2.values,ds.ak_pressure.values #read_sounding_positions(soundings_path)
    ...
    for i in range(0,len(release_time)):
        ...
        pathnames_dir_temp=f'{output_path}/{sim_start[i][0][:4]}_{sim_start[i][0][4:6]}/config/pathnames'
        ...
        pathnames_file=f'{output_path}/{sim_start[i][0][:4]}_{sim_start[i][0][4:6]}/config/pathnames/pathnames_{sim_start[i][0]}'      # uses dateinstead of number
