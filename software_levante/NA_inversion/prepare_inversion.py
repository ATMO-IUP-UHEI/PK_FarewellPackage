...
def calc_background_gosat(release_dir, molefrac_dir,num_parts,gosat_path,gosat_spath='',interp_method='linear', col_name='', xco2_col_name='',get_BG_xco2_val=True, plim=100):
...
def get_gosat_bg_xco2(start_date, end_date,flex_data_dir, gosat_dir,gosat_sdir,TM5_ds, TM5flux_path, molefrac_dir,molefrac_ds, num_parts=40000,BG='TM5'):
    ...
    for d in pd.date_range(start_date,end_date):
        ...
        if os.path.isfile(gosat_path):
            ...
            calc_background_gosat(release_dir, molefrac_dir,num_parts,gosat_spath, gosat_spath, col_name=f'{BG}_{molefrac_ds}_background',xco2_col_name=f'{BG}_{molefrac_ds}_xco2')
...
def process_flexpart_runs(start_date, end_date,flex_dir,gosat_dir,is_dir, TM5flux_dir,molefrac_dir, num_parts=40000,plim=100, ds_list=['RemoTeC_2.4.0+IS'], molefrac_ds='RemoTeC_2.4.0+IS',BG='TM5'):
    ...
    for ds in ds_list:
        ...
        get_gosat_bg_xco2(start_date, end_date,flex_data_dir, gosat_dir,gosat_sdir,ds, TM5flux_path, molefrac, molefrac_ds,BG=BG)
...
if __name__ == "__main__":  
    start_date, end_date =dt.date(2010,8,1), dt.date(2010,11,3)    # time period of measurements
    # data paths
    ...
    TM5flux_dir='/work/bb1170/RUN/b383736/data/PK_Flexpart/TM54DVar_with_diurnal_amplitude/TM54DVar_fluxes/'
    # path to high resolution (interpolated) scaling data
    scaling_data_path='/work/bb1170/RUN/b383736/data/PK_Flexpart/TM54DVar_with_diurnal_amplitude/high_res_total_scaling_RemoTeC+IS.nc'

    # FLEXPART
    flex_dir='/work/bb1170/RUN/b383736/data/PK_Flexpart/2months' # path to directory with RemoTeC and insitu Flexpart release subdirectories
    num_parts=40000     # number of particles per measurement

    # measurement data paths
    # path to soundingposition.csv files
    gosat_dir='/work/bb1170/RUN/b383736/data/test/GOSAT/PK_test'      
    is_dir=f'/work/bb1170/RUN/b383736/data/test/Obspack/PK_test'
    ...
    if PROCESS_FLEX_RUNS:
        process_flexpart_runs(start_date, end_date,flex_dir,gosat_dir,is_dir, TM5flux_dir,molefrac_dir,molefrac_ds=molefrac_bg_str,BG=BG)
        ...
        for month_start in pd.date_range(start_date, end_date, freq='MS'):
            ...
            if (gosat_footprint.release_day.dt.date.values==gosat_data.time.dt.date).all():
                ...
                if (gosat_footprint.release_num.values==gosat_data.release_num).all():
                    ...
                    cols=['release_time', 'release_lat', 'release_lon','xco2', 'xco2_err',
                        f'{BG}_{molefrac_bg_str}_background', f'{BG}_{molefrac_bg_str}_xco2',f'{BG}_{molefrac_bg_str}_xco2_interpolated', 'frac_remaining']
                    for col in cols:
    ...
    if COARSEN_HIGH_RES_FOOTPRINT:
        ...
        gosat_cols=['release_num', 'release_lat', 'release_lon', 'xco2', 'xco2_err', f'{BG}_{molefrac_bg_str}_background', f'{BG}_{molefrac_bg_str}_xco2',f'{BG}_{molefrac_bg_str}_xco2_interpolated', 'frac_remaining']
