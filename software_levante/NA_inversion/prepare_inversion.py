...
def calc_background_gosat(release_dir, molefrac_dir,num_parts,gosat_path,gosat_spath='',interp_method='linear', col_name='', xco2_col_name='',get_BG_xco2_val=True, plim=100,SAK= False):
    ...
    for date in pd.date_range(date_min,date_max):
        ...
        if firstFile:
            bg_data=xr.open_dataset(f'{molefrac_dir}xco2_mean_{date_str}.nc')
            firstFile=False
        else:
    ...
    if not SAK:
        ...
        bg_data=xr.open_dataset(f'{molefrac_dir}xco2_mean_{release_day_str}.nc')
        bg_data=bg_data.assign_coords(boundaries=bg_data.boundaries, levels=bg_data.levels).squeeze()
        # determine where plim is compared to boundaries, in which level
        upper_b=((bg_data.p_boundary<plim).idxmax(dim='boundaries')) # upper boundary of level =level+1
        ...
        # returns value from bg_data.p_diff where condition is True, so all levels that are not upper_b-1, otherwise fills in plim
        bg_data['p_diff']=bg_data.p_diff.where(bg_data.levels!=(upper_b-1), (plim-bg_data.p_boundary.sel(boundaries=upper_b)))#.sel(levels=17)
        # drop lower levels
        bg_data=bg_data.where(bg_data.levels>=(upper_b-1), drop=True)
        # weigh mix with p_diff, sum over levels
        bg_data['xco2_partial']=(bg_data.mix*bg_data.p_diff).sum(dim='levels')/plim
    ...
    if SAK:
        ...
        BG_xco2_val=[]
        Axco2=[]
        for i in range(0,len(gosat_data.xco2)):
            temp=bg_data.sel(latitude=gosat_data.latitude[i],longitude=gosat_data.longitude[i],times=gosat_data.time[i], method='nearest')
            val.append(background_part[i].item())
            if get_BG_xco2_val:
                # get xco2 value from TM5_4Dvar dataset
                BG_xco2_val.append(temp.xco2.values)
            xco2=0
            marker=False
            gosat_temp=gosat_data.isel(sounding_dim=i)
            gosat_temp['pressure_levels']=gosat_temp.pressure_levels*100
            for k in reversed(range(len(gosat_data.layer_dim))):
                gosat_top_pressure=gosat_temp.pressure_levels.values[k]
                gosat_bottom_pressure=gosat_temp.pressure_levels.values[k+1]
                A_i=gosat_temp.xco2_averaging_kernel.values[k]
                if max(temp.p_boundary.values)<gosat_bottom_pressure:
                    marker=True
                    lower_level=0
                else:
                    lower_level=len(temp.where(temp.p_boundary>=gosat_bottom_pressure,drop=True).p_boundary)-1
                upper_level=len(temp.where(temp.p_boundary>=gosat_top_pressure,drop=True).p_boundary)-1
                for bg_level in range(lower_level,upper_level+1):
                    p_1=min(temp.p_boundary.values[bg_level],gosat_bottom_pressure)
                    p_2=max(temp.p_boundary.values[bg_level+1],gosat_top_pressure)
                    p_diff = p_1-p_2
                    co2=temp.isel(levels=bg_level).mix.values
                    xco2+=co2*A_i*p_diff  
            if marker:
                xco2=xco2/(max(temp.p_boundary.values)-min(gosat_temp.pressure_levels.values))  
            else:
        ...
        if get_BG_xco2_val:
            # add data
            gosat_data[col_name]=val
            gosat_data[xco2_col_name]=BG_xco2_val
            gosat_data['BG_A*xco2']=Axco2
            gosat_data.to_csv(gosat_spath, index=False)
        else:
            # add data
            gosat_data[col_name]=val
        ...
        for i in range(0,len(gosat_data)):
            ...
            if get_BG_xco2_val:
                # get xco2 value from TM5_4Dvar dataset
                BG_xco2_val.append(temp.xco2.values[0])
        if get_BG_xco2_val:
            add_to_gosat_sounding_data([val, BG_xco2_val], [col_name, xco2_col_name],gosat_path,gosat_spath)
        else:
            add_to_gosat_sounding_data([val], [col_name],gosat_path,gosat_spath)  
        return val

def get_gosat_bg_xco2(start_date, end_date,flex_data_dir, gosat_dir,gosat_sdir,TM5_ds, TM5flux_path, molefrac_dir,molefrac_ds, num_parts=40000,BG='TM5',SAK=False):
    ...
    for d in pd.date_range(start_date,end_date):
        ...
        if not os.path.exists(release_dir):
            warn=f'WARNING: The directory {release_dir} does not exist. This directory will be skipped during processing.'
            warnings.warn(warn)
            continue
        if SAK:
...
def calc_interpolated_TM5_meas_values(start_date, end_date, bg_dir, gosat_dir, is_dir, bg_str='RemoTeC_2.4.0+IS', BG='TM5'):
    ...
    def interpolate_co2_point(row):
        temp = bg_data.mix.interp(
            times=row['time'],
            latitude=row['latitude'],
            longitude=row['longitude'],
            method='linear'  # or 'nearest' if needed
        )
        val = temp.swap_dims({'levels':'p_level'}).sel(p_level=row['p_intake_height[hPa]'], method="nearest")
        return val.values.item()  # extract scalar
    
    for date in pd.date_range(start_date, end_date):
        # read data
        bg_data=xr.open_dataset(f'{bg_dir}xco2_mean_{date.strftime("%Y%m%d")}.nc').squeeze()
        # gosat
        ...
        if os.path.isfile(gosat_path):  # check if file exists, there are some days without measurements
            ...
            gosat_csv[f'{BG}_{bg_str}_xco2_interpolated'] = gosat_csv.apply(interpolate_xco2_point, axis=1)
            # save csv files
        ...
        if os.path.isfile(is_path):
            ...
            is_csv[f'{BG}_{bg_str}_co2_interpolated'] = is_csv.apply(interpolate_co2_point, axis=1)
...
def calc_TM5_background_for_ISmeas(release_dir, molefrac_dir,num_parts,is_path,is_spath='',interp_method='linear', col_name='', co2_col_name=''):
    ...
    for i in range(0,len(is_data)):
        temp=bg_data.sel(latitude=is_data.latitude[i],longitude=is_data.longitude[i],times=is_data.time[i], method='nearest')
        # surface pressure
        ...
        BG_co2_val.append(temp.mix.sel(levels=p_level).item())
    # add to is_data df
    is_data['p_intake_height[hPa]']=np.array(p_intake_height)/100
    is_data[co2_col_name]=BG_co2_val
    print(f'saving to {is_spath}')
    is_data.to_csv(is_spath, index=None)
    return

def process_flexpart_runs(start_date, end_date,flex_dir,gosat_dir,is_dir, TM5flux_dir,molefrac_dir, num_parts=40000,plim=100, ds_list=['RemoTeC_2.4.0+IS'], molefrac_ds='RemoTeC_2.4.0+IS',BG='TM5',SAK=False):
    ...
    for ds in ds_list:
        ...
        molefrac=f"{molefrac_dir}/{molefrac_ds}/"
        ...
        for date in pd.date_range(start_date, end_date):
            ...
            calc_TM5_background_for_ISmeas(release_dir, molefrac,num_parts,is_path,is_spath, col_name=f'{BG}_{molefrac_ds}_background',co2_col_name=f'{BG}_{molefrac_ds}_co2')
        # get interpolated TM5-4DVar molefractions
        calc_interpolated_TM5_meas_values(start_date, end_date, molefrac, gosat_sdir, is_sdir, molefrac_ds,BG=BG)
...
if __name__ == "__main__":  
    ...
    if PREP_TM5_4DVAR_REF_FLUXES:
        ...
        for temp in [TM5_str, 'prior']:
            ...
            region=[12,56,-134,-62] # [2,66,-146,-50]  # somewhat larger than Flexpart region 12,56,-134,-62
            res=2
    ...
    if GET_IS_BG_CO2:
        for date in pd.date_range(start_date, end_date):
            ...
            if not os.path.isfile(f'{release_dir}DF_last_positions.pkl'): # create last_positions file if it doesnt exists
                print('create DF_last_positions.pkl')
                createLastPositionsDF(release_dir)
            calc_TM5_background_for_ISmeas(release_dir, TM5_dir,num_parts,is_path,is_spath, col_name=f'TM5_{TM5_str}_background',co2_col_name=f'TM5_{TM5_str}_co2')
    if GET_XCO2_VALS:
        # path to TM5-4DVar fluxes, used for enhancement estimate with 1x1 flux*footprint
        TM5flux_path_1x1=f'{TM5flux_dir}/flux_1x1_{TM5_str}_cut.nc'    
        get_gosat_bg_xco2(start_date, end_date,f'{flex_dir}/RemoTeCv240', gosat_dir,gosat_csv_sdir,TM5_str, TM5flux_path_1x1, TM5_dir)
    if GET_INTERP_TM5_VALS:
        calc_interpolated_TM5_meas_values(start_date, end_date, TM5_dir, gosat_csv_sdir, is_csv_dir, TM5_str)
    ...
    if GET_HIGH_RES_FOOTPRINTS:
        for month_start in pd.date_range(start_date, end_date, freq='MS'):
            ...
            for f in is_files:
                ...
                ds.append(temp)
            is_footprint=xr.concat(ds, dim='pointspec', data_vars='all', join='outer')
            ...
            if (is_footprint.release_day.dt.date.values==is_data.time.dt.date).all():
                ...
                if (is_footprint.release_num.values==is_data.release_num).all():
                    ...
                    #    'TM5_RemoTeC_2.4.0+IS_background', 'p_intake_height[hPa]','TM5_RemoTeC_2.4.0+IS_co2','TM5_RemoTeC_2.4.0+IS_co2_interpolated', 'frac_remaining']
                    cols=['release_time', 'file', 'release_lat', 'release_lon','co2_val[ppm]', 'elevation[masl]', 'intake_height[magl]',
                        f'{BG}_{molefrac_bg_str}_background', 'p_intake_height[hPa]',f'{BG}_{molefrac_bg_str}_co2',f'{BG}_{molefrac_bg_str}_co2_interpolated', 'frac_remaining']
                    for col in cols:
            ...
            for f in gosat_files:
                ...
                temp['release_day']=pd.to_datetime(f[-36:-28])
                ds.append(temp)
            gosat_footprint=xr.concat(ds, dim='pointspec', data_vars='all', join='outer')
            ...
            if (gosat_footprint.release_day.dt.date.values==gosat_data.time.dt.date).all():
                ...
                if (gosat_footprint.release_num.values==gosat_data.release_num).all():
                    ...
                    #    'TM5_RemoTeC_2.4.0+IS_background', 'TM5_RemoTeC_2.4.0+IS_xco2','TM5_RemoTeC_2.4.0+IS_xco2_interpolated', 'frac_remaining']
                    cols=['release_time', 'release_lat', 'release_lon','xco2', 'xco2_err',
                        f'{BG}_{molefrac_bg_str}_background', f'{BG}_{molefrac_bg_str}_xco2','BG_A*xco2',f'{BG}_{molefrac_bg_str}_xco2_interpolated', 'frac_remaining']
                    for col in cols:
    ...
    if ADDITIVE_DIURNAL_CYCLE:
        ...
        for dir_str in ['insitu', 'RemoTeCv240']:  #
            for month_start in pd.date_range(start_date, end_date, freq='MS'):
                ...
                if dir_str=='insitu':
                    data['real_co2_val[ppm]']=data['co2_val[ppm]']
                    data['co2_val[ppm]'] = data['co2_val[ppm]']-diurnal_offset
                if dir_str=='RemoTeCv240':
                    data['real_xco2']=data['xco2'] 
                    data['xco2'] = data['xco2']-diurnal_offset


                # save dataset
                spath=f'{flex_dir}/{dir_str}/prep_footprints/diurnal_amplitude_weekly/high_res_scaled_footprints_{month_start.strftime("%Y_%m")}_weekly.nc'
                if not os.path.isdir(f'{flex_dir}/{dir_str}/prep_footprints/diurnal_amplitude_weekly'):
                    os.makedirs(f'{flex_dir}/{dir_str}/prep_footprints/diurnal_amplitude_weekly')
    ...
    if COARSEN_HIGH_RES_FOOTPRINT:
        ...
        #gosat_cols=['release_num', 'release_lat', 'release_lon', 'xco2', 'xco2_err', 'TM5_RemoTeC_2.4.0+IS_background', 'TM5_RemoTeC_2.4.0+IS_xco2','TM5_RemoTeC_2.4.0+IS_xco2_interpolated', 'frac_remaining']
        is_cols=['release_num','release_day','release_time','file','release_lat','release_lon','co2_val[ppm]','elevation[masl]',
                 'intake_height[magl]',f'{BG}_{molefrac_bg_str}_background','p_intake_height[hPa]',f'{BG}_{molefrac_bg_str}_co2',f'{BG}_{molefrac_bg_str}_co2_interpolated','frac_remaining']
        gosat_cols=['release_num', 'release_lat', 'release_lon', 'xco2', 'xco2_err', f'{BG}_{molefrac_bg_str}_background', f'{BG}_{molefrac_bg_str}_xco2','BG_A*xco2',f'{BG}_{molefrac_bg_str}_xco2_interpolated', 'frac_remaining']
        ...
        for i in range(0,len(dir_str)):
            dir_path=f'{flex_dir}/{dir_str[i]}/prep_footprints/diurnal_amplitude_weekly'   
